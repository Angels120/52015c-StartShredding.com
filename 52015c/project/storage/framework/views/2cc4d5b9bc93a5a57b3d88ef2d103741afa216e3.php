            	...
            	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 left-table">
					...
					<div class="bg-white p-0">
						<ul class="nav custom-tabs ffourNavs">
						    ...
						    <li class="<?php if($activeMenu==3){echo 'active';} ?>"><a href="<?php echo url('vendor/settings'); ?>?section=login-users">Users</a></li>
					     	...
					     	<div class="bg-white">
								<div class="panel-body-custom tableContainParent">
									...
									<div id="addUserAccountBox" style ="<?=$formStyle?>">
										<div class="col-md-12">
											<form method="POST" id="addUserForm" action="<?php echo url('vendor/login-accounts'); ?>" class="form-horizontal form-label-left ajaxForm" enctype="multipart/form-data">
					                            <?php echo e(csrf_field()); ?>
...
<script>
    ...
    $(document).on("click","#addBankAccount",function(){
     ...
     $(document).on("click","#addUserAccountTrigger",function(){
		$("#addUserForm")[0].reset();
		window.location.href = "<?php echo url('vendor/settings'); ?>?section=login-users&action=add";
    });
        ...
        $.post(formData.attr("action"), formData.serialize(), function(r) {
            if (r != "")
            {
                ...
                if (data.status == "error") {
					...
					if(type == "user") {
						window.location.href="<?php echo url('vendor/settings'); ?>?section=login-users";
					}					
                }
            } else {
