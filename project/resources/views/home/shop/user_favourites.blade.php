...
<script>
    ...
    function addToShoppingCart(el){
        ...
        $.ajax({
            ...
            {
                alert('success', 'Success!', 'Cart updated successfully. <a href="{{ route("shop.cart") }}"><b><i class="fa fa-cart-plus cart-icon" style="margin-top: 0px; padding-top: 0px;"></i> Go to cart</b></a>');
            },
            error: function(xhr) {
                    ...
                    for (var i = 0; i < response.length; i++) {
                        ...
                        trows += '<tr id="' + response[i].id + '">' +
                            ...
                            '<td class="v-align-left"><a ' +
                                    'href="{{url("shop-product")}}/' + response[i].product.id + '/' + response[i].product.title.toLowerCase().replace(" ", "-") + '"' +
