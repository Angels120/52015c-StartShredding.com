<?php $__env->startSection('title', '| ' . $category_current['name']); ?>
        ...
        <div class="section-padding product-filter-wrapper wow fadeInUp">
            ...
            <div class="container inner-block">
                ...
                <table id="featuredTable" class="table table-striped tabele-bordered" style="margin-top:20px;">
                    ...
                    <tbody>
                        <?php $__currentLoopData = $featured_products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $featured_product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr>
                            <td><a
                                    href='<?php echo e(url('product') . '/' . $featured_product->id . '/' . str_replace(' ', '-', strtolower($featured_product->title))); ?>'><?php echo e($featured_product->title); ?></a>
                                <input type='hidden' class='_token' name='_token' value='<?php echo e(csrf_token()); ?>'>
                                ...
                                <input type='hidden' class='price' name='price'
                                    value='<?php echo e(number_format((float)$featured_product->price, 2, '.', '')); ?>'>
                                <input type='hidden' class='title' name='title'
                                    value='<?php echo e(str_replace(' ', '-', strtolower($featured_product->title))); ?>'>
                                <input type='hidden' class='product' name='product' value='<?php echo e($featured_product->id); ?>'>
                                <input type='hidden' class='cost' name='cost' value='<?php echo e($featured_product->price); ?>'>
                                <input type='hidden' class='size' name='size'>
                            </td>
                            <td class='text-left'>$ <?php echo e(number_format((float)$featured_product->price, 2, '.', '')); ?>
                            ...
                            <td class='text-center icons'>
                                ...
                                <input class='number quantity' style='border-style: none;width: 17px;' type='text'
                                    id='<?php echo e($key); ?>' value='1' readonly>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <select name="first" id="subCategory" class="selectpicker" style="color: #0059B2;">
                                ...
                                <?php if($cat->id === $category_current['id'] || $cat->id === $category_current->subid['id']): ?>
...
<script>
    $(document).ready(function(e){
        $.getJSON("<?php echo e(route('get.child.categories')); ?>?cat_id=" + $("#subCategory").val(), function(values) {
            var options, option;

            var select_child = "<?php echo e($category_current->id); ?>";
            $("#childCategory").html("");
        ...
        var cat_current = "<?php echo e($category_current['id']); ?>";
        $.getJSON("<?php echo e(route('get.products')); ?>?cat_id=" + cat_current, function(values) {
            processProductList(values);
        });

        $("#subCategory").change(function(ev){
            var subCat = $(this).val();
            $.getJSON("<?php echo e(route('get.child.categories')); ?>?cat_id=" + subCat, function(values) {
                var options;
                $("#childCategory").html("");
            ...
            $.getJSON("<?php echo e(route('get.products')); ?>?cat_id=" + $(this).val(), function(values) {
                processProductList(values);
            });
        });
    ...
    function processProductList (values) {
        ...
        for (var i = 0; i < values.length; i++) {
            ...
            trows +=    "<tr><td><a href='<?php echo e(url('product')); ?>/" + values[i].id + "/" + values[i].title.toLowerCase().replace(' ', '-') + "'>" + values[i].title +
                        "</a>" +
                        "<input type='hidden' class='_token' name='_token' value='<?php echo e(csrf_token()); ?>'>" +
                        "<input type='hidden' class='uniqueid name='uniqueid'' value='<?php echo e(Session::get('uniqueid')); ?>'>" +
...
<script>
    $("#filter").validate({
        ...
        $("#searchProduct").keyup(function(e){
            ...
            if($("#filter").valid()){
                if($.trim($(this).val()) == ""){
                    $.getJSON("<?php echo e(route('get.products')); ?>?cat_id=6", function(values) {
                        processProductList(values);
                    });
                }
                else{
                    $.getJSON("<?php echo e(route('search.products')); ?>?search=" + $(this).val(), function(values) {
                        processProductList(values);
                    });
                }
