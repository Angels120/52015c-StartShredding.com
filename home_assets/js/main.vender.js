...
function getCart() {
    $.get(mainurl + '/home/cartupdate', function (response) {
        var total = 0;
        $("#goCart").html('');
...
function getDelete(id) {
    $.getJSON(mainurl + '/home/cartdelete/' + id, function (response) {
...
// function toAddCartTierFromTable(id) {
//
//     var token = { name: '_token', value: $('.token_'+id).val() };
//     var uniqueid = { name: 'uniqueid', value: $('.uniqueid_'+id).val() };
//     var quantity = { name: 'quantity', value: $('.quantity_'+id).val() };
//     var price = { name: 'price', value: $("input[name='tierPrice']:checked").val() };
//     var title = { name: 'title', value: $('.title_'+id).val() };
//     var product = { name: 'product', value: $('.product_'+id).val() };
//     var cost = { name: 'cost', value: $('.cost_'+id).val() };
//     var size = { name: 'size', value: $('.size_'+id).val() };
...
//                             "<button class='btn-remove' title='Remove This Item' type='submit' style='margin-top:-5px;'></button>" +
//                             "</form>" +
//                             "</td>" +
//                             "</tr>";
//                     });
...
//                         discount = (parseFloat(discount_value) / 100) * parseFloat(price);
//                     }
//
//
//                     var tax =(price + parseFloat(delivery_fee)) * 0.13;
//                     var grandTotal = ( parseFloat(round(tax,2)) + parseFloat(price)) + (parseFloat(delivery_fee) + parseFloat(donation_amount))- parseFloat(discount);
//
...
//                         $("#cartProductTable").find("#cartSummaryTax").html('$' + round(tax,2));
...
function toAddCartFromTable(id) {

    var token = { name: '_token', value: $('.token_'+id).val() };
    var uniqueid = { name: 'uniqueid', value: $('.uniqueid_'+id).val() };
    var quantity = { name: 'quantity', value: $('.quantity_'+id).val() };
    var price = { name: 'price', value: $('.price_'+id).val() };
    var title = { name: 'title', value: $('.title_'+id).val() };
    var product = { name: 'product', value: $('.product_'+id).val() };
    var cost = { name: 'cost', value: $('.cost_'+id).val() };
    var size = { name: 'size', value: $('.size_'+id).val() };
        ...
        success: function (data) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var trows, subtotal, totalTable;
                    $.each(response, function (i, product) {
                        trows += "<tr><td><a href='" + mainurl + "/shop-product/" + product.product + "/" + product.title.toLowerCase().replace(' ', '-') + "'>" + product.title +
                            "</a>" +
                            "</td>" +
                            "<td class='text-center'>" +
                            product.quantity +
