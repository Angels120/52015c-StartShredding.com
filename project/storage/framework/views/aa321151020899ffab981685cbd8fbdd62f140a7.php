<?php $__env->startSection('title','My Faves'); ?>
...
<script>
    ...
    function addToShoppingCart(el){
        var $tr = $(el).closest("tr");
        var uniqueid = "<?php echo e($uniqueid); ?>";
        var data = {
            _token: null,
            uniqueid: uniqueid,
        ...
        $.ajax({
            url: '<?php echo e(route("api.user.cartupdate")); ?>',
            type: 'post',
            dataType: "JSON",
            data: data,
            success: function (response)
            {
                alert('success', 'Success!', 'Cart updated successfully. <a href="<?php echo e(route("user.cart")); ?>"><b><i class="fa fa-cart-plus cart-icon" style="margin-top: 0px; padding-top: 0px;"></i> Go to cart</b></a>');
            },
            error: function(xhr) {
    ...
    function removeFromFav(el){
        ...
        if (confirmation) {
            $.ajaxSetup({ headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') }});
            $.ajax({
                url: '<?php echo e(route("api.user.product-favourite.get")); ?>/' + id,
                type: 'delete',
                dataType: "JSON",
                data: {
                    "id": id
    ...
    function fetch(limit, page, search) {
        ...
        $.ajaxSetup({ headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') }});
        $.ajax({
            url: '<?php echo e(route("api.user.product-favourite.get")); ?>?search=' + search + '&limit=' + limit + '&page=' + page,
            type: 'get',
            dataType: "JSON",
            success: function (response)
            {
                    ...
                    for (var i = 0; i < response.length; i++) {
                        ...
                        trows += '<tr id="' + response[i].id + '">' +
                            ...
                            '<td class="v-align-left"><a ' +
                                    'href="<?php echo e(url("product")); ?>/' + response[i].product.id + '/' + response[i].product.title.toLowerCase().replace(" ", "-") + '"' +
